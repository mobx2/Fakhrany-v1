    ...
    >
      <div className="max-w-6xl mx-auto">
        <div className="flex items-center justify-between mb-6">
          <h2 className="text-2xl font-bold text-[#111827]">All Categories</h2>
          <a
            href="#"
        ...
        <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6">
          {categories.map((category) => (
            <button
              ...
              className={`${
                category.bgColor
              } rounded-3xl p-5 flex flex-col items-center justify-center gap-3 transition-all duration-300 active:scale-95 min-h-36 border-2 ${
                selectedCategory === category.id
                  ? "border-[#800000] shadow-lg scale-105"
                  : "border-white/50 hover:shadow-lg hover:-translate-y-1 hover:border-[#800000]"
              }`}
              aria-label={category.name}
              aria-pressed={selectedCategory === category.id}
            >
              <div className="text-4xl">{category.emoji}</div>
              <p className="text-xs font-semibold text-center text-[#111827] text-balance leading-tight">
                {category.name}
              </p>
            </button>
          ))}
